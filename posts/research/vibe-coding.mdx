...
One problem with prior complex tools is that a specific task would be buried behind several dropdowns. Many companies have experimented with AI not to replace the current UI, but rather as a guide to show how to use the AI. At Ramp, the [AI agent clicks through the UI](https://x.com/tryramp/status/1792659194996281478), while for Stitch, a new node-based visual programming prototyping tool similar to Origami - [AI is used to create the abstracted primitive building block](https://stitchdesign.app/)s (video below).
